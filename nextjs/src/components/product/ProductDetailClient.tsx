...
import { reviewClientApi, productClientApi } from '@/lib/api'
import { useIsMobile } from '@/hooks/useIsMobile'
import type { Product, ProductVariant, ReviewResponse, CanReviewResponse, SiteSettings } from '@/types'
        ...
        <div style={{ display: 'flex', gap: 1 }}>
          {[1, 2, 3, 4, 5].map(i => {
            ...
            const anim = (name: string): React.CSSProperties => play && active ? {
              animationName: name, animationDuration: '1s',
              animationTimingFunction: 'ease-in-out', animationFillMode: 'forwards', animationDelay: delay,
            } : {}
            return (
              <svg key={i} width="19" height="19" viewBox="0 0 32 32" style={{ overflow: 'visible' }}>
                ...
                <g strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <g transform="translate(16,16) rotate(180)">
                    <polygon points="0,15 4.41,6.07 14.27,4.64 7.13,-2.32 8.82,-12.14 0,-7.5 -8.82,-12.14 -7.13,-2.32 -14.27,4.64 -4.41,6.07"
...
export default function ProductDetailClient({ product, settings, parentCatSlug, parentCatName }: Props) {
  ...
  const [selectedVariant, setSelectedVariant] = useState<ProductVariant | null>(hasVariants ? activeVariants[0] : null)
  ...
  useEffect(() => {
    ...
    reviewClientApi.list(product.slug).then(r => { setReviews(r); setReviewsLoaded(true) }).catch(() => {})
    if (user) reviewClientApi.canReview(product.slug).then(setCanReview).catch(() => {})
  }, [activeTab, reviewsLoaded, user, product.slug])

  useEffect(() => {
    const vid = selectedVariant?.id ?? 0
    const already = localStorage.getItem(`stock-notify-${product.id}-${vid}`) === '1'
    setNotifySubmitted(already)
    setNotifyOpen(false)
    setNotifyEmail(user?.email ?? '')
  }, [selectedVariant, user?.email, product.id])

  const effectivePrice = selectedVariant ? Number(selectedVariant.price) : Number(product.basePrice)
  const effectiveStock = selectedVariant ? selectedVariant.availableStock : product.availableStock

  const handleAddToCart = () => {
    if (hasVariants && !selectedVariant) { toast.error('Lütfen bir paket seçin'); return }
    ...
    dispatch(addToCart({
      productId: product.id, variantId: selectedVariant?.id, variantLabel: selectedVariant?.label,
      name: product.name, slug: product.slug, brandName: product.brandName,
      basePrice: effectivePrice, unit: product.unit,
      availableStock: effectiveStock, primaryImageUrl: product.primaryImageUrl, quantity: qty,
    }))
  ...
  const handleNotifyStock = async () => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { toast.error('Geçerli bir e-posta girin'); return }
    setNotifySaving(true)
    try {
      await productClientApi.notifyStock(product.id, { email, ...(selectedVariant ? { variantId: selectedVariant.id } : {}) })
      localStorage.setItem(`stock-notify-${product.id}-${selectedVariant?.id ?? 0}`, '1')
  ...
  const handleEditSubmit = async (r: ReviewResponse) => {
    ...
    try {
      const updated = await reviewClientApi.update(product.slug, r.id, editStar, editComment)
  ...
  const handleDeleteReview = async (reviewId: number) => {
    try {
      await reviewClientApi.delete(product.slug, reviewId)
    ...
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '0 12px' : '0 20px' }}>
        ...
        <nav style={{ padding: '16px 0 10px', display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
          ...
          {parentCatSlug && parentCatName && (
            <>
              <span style={{ color: 'var(--text3)', fontSize: 11 }}>›</span>
              <Link href={`/urunler?slug=${parentCatSlug}`} style={{ fontSize: 13, color: 'var(--text2)' }}>{parentCatName}</Link>
            </>
          )}
          <span style={{ color: 'var(--text3)', fontSize: 11 }}>›</span>
          <Link href={`/urunler?slug=${product.categorySlug}`} style={{ fontSize: 13, color: 'var(--text2)' }}>{product.categoryName}</Link>
          <span style={{ color: 'var(--text3)', fontSize: 11 }}>›</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 20 : 36, padding: '8px 0 36px', alignItems: 'start' }}>
          ...
          <div>
            <div style={{
              ...
              fontSize: isMobile ? 80 : 110, marginBottom: 12, position: 'relative', overflow: 'hidden',
            }}>
              {(() => {
                const url = activeImg ? imgUrl(activeImg.imageUrl) : undefined
                return url
                  ? <Image src={url} alt={product.name} fill style={{ objectFit: 'contain', padding: 16 }} sizes="(max-width: 768px) 100vw, 50vw" priority />
                  : <span>{THUMB_EMOJIS[thumbIdx]}</span>
              })()}
            </div>
            {hasRealImages ? (
              <div style={{ display: 'flex', gap: 9, flexWrap: 'wrap' }}>
                {productImgs.map((img, i) => {
                  const url = imgUrl(img.imageUrl)
                  if (!url) return null
                  return (
          ...
          <div>
            ...
            {hasVariants && (
              <div style={{ marginBottom: 18 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text2)', marginBottom: 8 }}>Paket Seçin <span style={{ color: 'var(--primary)' }}>*</span></div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {activeVariants.map(v => {
                    const isSelected = selectedVariant?.id === v.id
                    const oos = v.availableStock <= 0
                    return (
                      <button key={v.id} onClick={() => { if (!oos) { setSelectedVariant(isSelected ? null : v); setQty(1) } }} disabled={oos}
                        style={{ padding: '8px 16px', borderRadius: 'var(--r)', border: `2px solid ${isSelected ? 'var(--primary)' : 'var(--border)'}`, background: isSelected ? 'var(--primary-bg)' : 'var(--bg2)', color: oos ? 'var(--text3)' : isSelected ? 'var(--primary)' : 'var(--text)', fontSize: 13, fontWeight: 700, cursor: oos ? 'not-allowed' : 'pointer', opacity: oos ? 0.5 : 1, transition: '0.15s', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                        <span>{v.label}</span>
                        <span style={{ fontSize: 12, fontWeight: 800, color: isSelected ? 'var(--primary)' : 'var(--text2)' }}>₺{Number(v.price).toFixed(2)}</span>
                        {oos && <span style={{ fontSize: 10, color: 'var(--text3)' }}>Tükendi</span>}
                      </button>
                    )
                  })}
                </div>
            ...
            <div style={{ marginBottom: 18 }}>
              {!hasVariants && product.activeDiscount ? (() => {
                ...
                const newPrice = disc.discountType === 'PERCENT' ? effectivePrice * (1 - disc.discountValue / 100) : effectivePrice - disc.discountValue
                return (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                    ...
                    <span style={{ background: 'var(--primary)', color: '#fff', fontSize: 12, fontWeight: 800, padding: '4px 10px', borderRadius: 20 }}>{disc.label} İndirim</span>
                  </div>
                )
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', border: '2px solid var(--border2)', borderRadius: 'var(--r)', overflow: 'hidden' }}>
                ...
                <button onClick={() => { if (qty >= effectiveStock) { toast.error('Stokta yeterli ürün yok'); return } setQty(qty + 1) }} style={{ width: 38, height: 38, background: 'var(--primary)', color: '#fff', fontSize: 20, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', cursor: 'pointer' }}>+</button>
              </div>
              {effectiveStock > 0 && effectiveStock < 10 && (
                <span style={{ fontSize: 12, fontWeight: 700, color: '#d97706', background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: 20, padding: '2px 10px' }}>⚠️ Son {effectiveStock} stok!</span>
              )}
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
              {effectiveStock > 0 ? (
                <button onClick={handleAddToCart} style={{ width: '100%', background: addedToCart ? '#22c55e' : 'var(--primary)', color: '#fff', fontSize: 15, fontWeight: 700, padding: 14, borderRadius: 'var(--r)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: '0.2s', boxShadow: '0 4px 12px rgba(220,38,38,.35)', border: 'none', cursor: 'pointer' }}>
                  {addedToCart ? '✅ Sepete Eklendi!' : '🛒 Sepete Ekle'}
                </button>
              ) : notifySubmitted ? (
                <div style={{ width: '100%', padding: '14px 16px', borderRadius: 'var(--r)', background: '#ecfdf5', border: '1px solid #a7f3d0', color: '#065f46', fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, textAlign: 'center' }}>
                  ✅ Stok geldiğinde mail ile dönüş yapacağız
                </div>
              ) : !notifyOpen ? (
                <button onClick={() => setNotifyOpen(true)} style={{ width: '100%', background: 'var(--primary)', color: '#fff', fontSize: 15, fontWeight: 700, padding: 14, borderRadius: 'var(--r)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: '0.2s', boxShadow: '0 4px 12px rgba(220,38,38,.35)', border: 'none', cursor: 'pointer' }}>
                  🔔 Stoğa Gelince Haber Ver
                </button>
              ) : (
                <div style={{ width: '100%', padding: 14, borderRadius: 'var(--r)', background: 'var(--bg2)', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)' }}>🔔 Stoğa gelince haber ver</div>
                  <div style={{ fontSize: 12, color: 'var(--text2)', lineHeight: 1.5 }}>E-posta adresinizi girin, ürün tekrar stoğa girdiğinde bildirim göndereceğiz.</div>
                  <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 8 }}>
                    <input type="email" value={notifyEmail} onChange={e => setNotifyEmail(e.target.value)} placeholder="E-posta adresiniz" disabled={notifySaving}
                      style={{ flex: 1, minWidth: 0, padding: '10px 12px', border: '1px solid var(--border)', borderRadius: 8, fontSize: 14, background: 'var(--bg)', color: 'var(--text)', outline: 'none' }} />
                    <button onClick={handleNotifyStock} disabled={notifySaving} style={{ background: 'var(--primary)', color: '#fff', fontSize: 14, fontWeight: 700, padding: '10px 18px', borderRadius: 8, border: 'none', cursor: notifySaving ? 'wait' : 'pointer', whiteSpace: 'nowrap' }}>
                      {notifySaving ? 'Kaydediliyor…' : 'Haber Ver'}
                    </button>
                  </div>
                  <button onClick={() => setNotifyOpen(false)} disabled={notifySaving} style={{ alignSelf: 'flex-end', background: 'transparent', color: 'var(--text3)', fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer', padding: 0 }}>Vazgeç</button>
                </div>
              )}
              <a href={`https://wa.me/${contactPhone}?text=${encodeURIComponent(`Merhaba, "${product.name}" ürünü hakkında bilgi almak istiyorum.`)}`}
                target="_blank" rel="noopener noreferrer"
                style={{ width: '100%', background: '#22c55e', color: '#fff', fontSize: 15, fontWeight: 700, padding: 14, borderRadius: 'var(--r)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: '0.2s', boxShadow: '0 4px 12px rgba(34,197,94,.3)', textDecoration: 'none' }}>
                💬 Satıcıya Sor
              </a>
            </div>
        ...
        <div style={{ marginBottom: 36 }}>
          <div style={{ display: 'flex', gap: 0, borderBottom: '2px solid var(--border)', marginBottom: 24 }}>
            {(['desc', 'specs', 'reviews'] as const).map(t => (
              <button key={t} onClick={() => setActiveTab(t)} style={{ padding: '12px 22px', fontSize: 14, fontWeight: 600, color: activeTab === t ? 'var(--primary)' : 'var(--text2)', background: 'none', border: 'none', cursor: 'pointer', borderBottom: activeTab === t ? '2px solid var(--primary)' : '2px solid transparent', marginBottom: -2, transition: '0.18s' }}>
          ...
          {activeTab === 'desc' && (
            <div>
              ...
              <div style={{ background: 'var(--primary-bg)', borderLeft: '4px solid var(--primary)', padding: '14px 18px', borderRadius: '0 var(--r) var(--r) 0', margin: '18px 0', fontSize: 14, color: 'var(--text2)', lineHeight: 1.6 }}>
                🏆 {settings.brandPart1}{settings.brandPart2}, en kaliteli pet ürünlerini toptan fiyatlarla sunan güvenilir platformunuzdur.
              </div>
              <p style={{ fontSize: 15, color: 'var(--text2)', lineHeight: 1.75, marginBottom: 14 }}>
                <strong style={{ color: 'var(--text)' }}>Toptan alım avantajı:</strong> Bu ürün özel toptan fiyatlarıyla sunulmaktadır.
              </p>
            </div>
          )}
            ...
            <div>
                ...
                <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--r2)', padding: '20px 24px', marginBottom: 20 }}>
                  ...
                  <div style={{ marginBottom: 12 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      {[1,2,3,4,5].map(s => (
                        <button key={s} type="button" onClick={() => setStarInput(s)} style={{ fontSize: 28, background: 'none', border: 'none', cursor: 'pointer', color: s <= starInput ? '#f59e0b' : 'var(--border2)', lineHeight: 1, padding: 0, transition: '0.1s' }}>★</button>
                      ))}
                    </div>
                  </div>
                  <div style={{ marginBottom: 14 }}>
                    <div style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 6 }}>Yorum <span style={{ fontSize: 11, color: 'var(--text3)' }}>(opsiyonel)</span></div>
                    <textarea value={commentInput} onChange={e => setCommentInput(e.target.value)} rows={3} placeholder="Ürün hakkında düşüncelerinizi paylaşın..."
                  ...
                  <button disabled={submitting || starInput === 0} onClick={async () => {
                    ...
                    setSubmitting(true)
                    try {
                      await reviewClientApi.create(product.slug, starInput, commentInput)
                      setStarInput(0); setCommentInput(''); setReviewsLoaded(false)
              ...
              {reviews.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {reviews.map(r => {
                    const isOwn = user != null && r.userId === (user as any).id
                      ...
                      <div key={r.id} style={{ background: 'var(--bg2)', border: `1px solid ${isOwn ? 'rgba(220,38,38,.25)' : 'var(--border)'}`, borderRadius: 'var(--r2)', padding: '18px 20px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <div>
                              <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)' }}>
                                {r.userName}{isOwn && <span style={{ marginLeft: 6, fontSize: 11, color: 'var(--primary)', fontWeight: 600 }}>( Siz )</span>}
                              </div>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            {isOwn && !isEditing && (
                              <>
                                <button onClick={() => { setEditingReviewId(r.id); setEditStar(r.rating); setEditComment(r.comment ?? '') }} title="Düzenle" style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 15, padding: '2px 4px', color: 'var(--text3)', lineHeight: 1 }}>✏️</button>
                                <button onClick={() => handleDeleteReview(r.id)} title="Sil" style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 15, padding: '2px 4px', color: 'var(--text3)', lineHeight: 1 }}>🗑️</button>
                              </>
                            )}
                          </div>
                        </div>
                        {isEditing ? (
                          <div>
                            <div style={{ display: 'flex', gap: 4, marginBottom: 10 }}>
                              {[1,2,3,4,5].map(s => (
                                <button key={s} type="button" onClick={() => setEditStar(s)} style={{ fontSize: 26, background: 'none', border: 'none', cursor: 'pointer', color: s <= editStar ? '#f59e0b' : 'var(--border2)', lineHeight: 1, padding: 0, transition: '0.1s' }}>★</button>
                              ))}
                            </div>
                            <textarea value={editComment} onChange={e => setEditComment(e.target.value)} rows={3} placeholder="Yorumunuzu düzenleyin..."
                            ...
                            <div style={{ display: 'flex', gap: 8 }}>
                              <button disabled={editSubmitting || editStar === 0} onClick={() => handleEditSubmit(r)} style={{ padding: '8px 20px', background: editStar ? 'var(--primary)' : 'var(--bg3)', color: editStar ? '#fff' : 'var(--text3)', border: 'none', borderRadius: 'var(--r)', fontSize: 13, fontWeight: 700, cursor: editStar ? 'pointer' : 'not-allowed' }}>
                                {editSubmitting ? 'Kaydediliyor...' : 'Kaydet'}
                              </button>
                              <button onClick={() => setEditingReviewId(null)} style={{ padding: '8px 16px', background: 'var(--bg3)', color: 'var(--text2)', border: '1px solid var(--border)', borderRadius: 'var(--r)', fontSize: 13, cursor: 'pointer' }}>İptal</button>
                            </div>
                          </div>
                        ) : (
                          <>
