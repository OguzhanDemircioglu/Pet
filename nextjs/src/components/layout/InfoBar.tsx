...
export default function InfoBar({ settings }: Props) {
  const [cur, setCur] = useState(0)
  const { contactPhone, contactEmail, contactHours } = settings

  const displayPhone = formatDisplayPhone(contactPhone)
    ...
    },
    {
      content: (
        <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ color: 'rgba(255,255,255,.6)' }}>&nbsp;·&nbsp; {contactHours || 'Haftaiçi 09:00–18:00'}</span>
        </span>
      ),
    },
    {
