...
public class ProductService {
    ...
    public ProductResponse getBySlug(String slug) {
        ...
        // Rating snapshot Product entity'sinde tutulur — review modülü güncellemesi event'le
        Double avg = p.getAvgRating() != null ? p.getAvgRating().doubleValue() : null;
        Integer cnt = p.getReviewCount();
        return ProductResponse.fromWithDetails(p, dm.get(p.getId()), avg, cnt != null && cnt > 0 ? cnt : null);
    }
    ...
    /**
     * Verilen ürün ID listesini sırayı koruyarak FeaturedProductDto'lara dönüştürür.
     * Best-seller composer (üst seviye controller) tarafından çağrılır — order modülünden
     * ID'ler gelir, catalog yalnızca enrichment yapar (cross-module dep yok).
     */
    public List<FeaturedProductDto> getFeaturedByIds(List<Long> ids) {
    ...
    public ProductResponse update(Long id, CreateProductRequest req) {
        ...
        if (previousAvailable <= 0 && saved.getAvailableStock() > 0) {
            eventPublisher.publishEvent(
                    new com.petshop.catalog.api.events.StockRestoredEvent(saved.getId(), null));
        }
    ...
    private Map<Long, ProductResponse.ActiveDiscountDto> buildDiscountMap() {
        ...
        productRepository.findProductIdsWithBrandDiscounts(now)
                .forEach(row -> map.merge(
                        (Long) row[0],
                        toDto((String) row[1], ((Enum<?>) row[2]).name(), (BigDecimal) row[3]),
                        this::better));
