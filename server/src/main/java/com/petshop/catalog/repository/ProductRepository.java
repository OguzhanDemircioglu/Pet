...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    // NOT: Best-seller hesabı OrderRepository.findBestSellerProductIdsByStatuses içinde
    // yer alır; catalog modülü OrderFacade.findBestSellerProductIds() ile ID listesi
    // alır, ardından findByIdsWithImages ile zenginleştirir.
    ...
    @Query("""
        ...
        FROM Product p
        JOIN BrandDiscount bd ON p.brand.id = bd.brandId
        WHERE p.isActive = true
    ...
    @Query("""
        ...
        FROM Product p
        JOIN CategoryDiscount cd ON p.category.id = cd.categoryId
        WHERE p.isActive = true
    ...
    @Query("""
        SELECT pd.productId, pd.name, pd.discountType, pd.discountValue
